...
public extension InteractionContext {
	...
	func scroll(_ element: Scrollable, direction: ElementAttributes.Direction, until otherElement: Element, is states: ElementAttributes.State..., maxTries: Int = 10) {
		var numberOfTries = 0
		repeat {
			let isInCorrectState = false // TODO
			guard !isInCorrectState else { return }
			numberOfTries += 1
			element.scroll(direction: direction)
